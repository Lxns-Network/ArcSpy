...
(() => {
    ...
    */
    ...
    function syncSongList() {
        console.log(i18n.songList.syncing);
        ...
        ).catch(error => alert(replaceVariables(i18n.songList.syncFailed, {
    ...
    */
    function getPlayerScores(friendCode, isSelf=false) {
        ...
        if (cachedPlayer === undefined) {
            alert(i18n.player.cacheNotFound);
            return [];
        }
        const userId = cachedPlayer.user_id;

        if (songList === undefined) {
            alert(i18n.songList.cacheNotFound);
            return [];
        }
        ...
        queryIntervalId = setInterval(() => {
            ...
            if (index >= sortedSongRatingList.length) {
                ...
                if (scores.length !== 0) {
                    processText.textContent = replaceVariables(i18n.query.entireSuccessResult, {
                        count: scores.length,
                        time: ((end - start) / 1000).toFixed()
                    });
                    ...
                    processText.textContent = replaceVariables(i18n.query.noScoreResult, {
                        count: scores.length,
                        time: ((end - start) / 1000).toFixed()
                    });
                    alert(i18n.query.noScoreAlert);
                    document.getElementById("arcspy-back-button").style.display = "flex";
                    document.getElementById("arcspy-interrupt-button").style.display = "none";
                    return [];
                }
            ...
            processText.textContent = replaceVariables(i18n.query.querying, {
                songId: song.key,
                difficulty: DIFFICULTY_NAME[song.index]
            });
            requestAPI('GET', `${endpoint}?song_id=${song.key}&difficulty=${song.index}&start=0&limit=30`).then((response) => {
                if (!response.success) {
                    clearInterval(queryIntervalId);
                    alert(replaceVariables(i18n.query.singleQueryFailed, {
                        songId: song.key,
                        difficulty: DIFFICULTY_NAME[song.index],
                ...
                if (scores.length >= maxScoreCount) {
                    ...
                    if (songMaxRating - playerMinRating <= 2) {
                        ...
                        if (songMaxRating < playerMinRating) {
                            const end = performance.now();
                            processText.textContent = replaceVariables(i18n.query.bestSuccessResult, {
    ...
    */
    function openQueryProcessModal() {
        ...
        const processText = arcSpyModal.createSpanModal(i18n.query.preparing);
        ...
        const interruptBtnModal = arcSpyModal.createBtnModal(i18n.query.saveAndInterrupt, 'light');
        interruptBtnModal.setAttribute("id", "arcspy-interrupt-button");
        interruptBtnModal.addEventListener('click', () => {
            clearInterval(queryIntervalId);
            alert(i18n.query.saveAndInterruptAlert);
            openScoreQueryModal();
        });
        modalAction.appendChild(interruptBtnModal);

        const uploadBtnModal = arcSpyModal.createBtnModal(i18n.upload.title, 'light');
        ...
        const backBtnModal = arcSpyModal.createBtnModal(i18n.return, 'light');
        ...
        arcSpyModal.setModalBody(i18n.query.title, modalBody);
    }
    ...
    */
    function openScoreQueryModal() {
        ...
        bodyModal.appendChild(arcSpyModal.createSpanModal(i18n.query.selectTargetPlayer));
        ...
        bodyModal.appendChild(arcSpyModal.createSpanModal(i18n.query.cacheNotice));

        const modalAction = arcSpyModal.getInitializedModalAction();

        const startBtnModal = arcSpyModal.createBtnModal(i18n.query.start);
        ...
        startBtnModal.addEventListener('click', () => {
            ...
            requestAPI('GET', 'user/me').then((response) => {
                if (!response.success) {
                    alert(replaceVariables(i18n.player.detailQueryFailed, { errorCode: response.error_code }))
                    return;
                }
                if (response.value.user_code !== friendCode && Date.now() - response.value.arcaea_online_expire_ts > 0) {
                    alert(i18n.player.requireOnline);
                    return;
                }
                ...
                if (!response.success) {
                    if (response.error_code === 604) {
                        if (!confirm(i18n.query.samePlayerWarning)) {
                            return;
                        }
                        useSongMeAPI = true;
                    } else if (response.error_code === 401) {
                        alert(i18n.query.playerNotFound);
                        return;
                    } else if (response.error_code !== 602) {
                        alert(replaceVariables(i18n.query.addFriendFailed, { errorCode: response.error_code }))
                        return;
                    }
                }
    ...
    */
    function openOptionModal() {
        ...
        bodyModal.appendChild(arcSpyModal.createSpanModal(i18n.config.songListUrl));
        ...
        bodyModal.appendChild(arcSpyModal.createSpanModal(i18n.config.scoreQueryInterval));
        ...
        bodyModal.appendChild(arcSpyModal.createSpanModal(i18n.config.saveCookie));
        ...
        bodyModal.appendChild(arcSpyModal.createSpanModal(i18n.config.saveCookieNotice));

        const modalAction = arcSpyModal.getInitializedModalAction();

        const saveBtnModal = arcSpyModal.createBtnModal(i18n.config.save);
        ...
        saveBtnModal.addEventListener('click', () => {
            validateSongListUrl(songListUrlInputModal.value).then((isLegalSongListUrl) => {
                if (!isLegalSongListUrl) {
                    alert(i18n.config.illegalSongListUrl);
                    return;
                } else {
            ...
            if (isNaN(parseInt(scoreQueryIntervalInputModal.value))) {
                alert(i18n.config.illegalScoreQueryInterval);
            } else {
                scoreQueryInterval = parseInt(scoreQueryIntervalInputModal.value);
                GM_setValue("scoreQueryInterval", scoreQueryInterval);
            }
        ...
        const resetBtnModal = arcSpyModal.createBtnModal(i18n.config.default, 'light');
        ...
        arcSpyModal.setModalBody(i18n.config.title, modalBody);
    }
    ...
    */
    function openUploadModal() {
        ...
        bodyModal.appendChild(arcSpyModal.createSpanModal(i18n.upload.selectTargetPlayer));
        ...
        bodyModal.appendChild(arcSpyModal.createSpanModal(i18n.upload.uploadNotice));

        const modalAction = arcSpyModal.getInitializedModalAction();

        const uploadBtnModal = arcSpyModal.createBtnModal(i18n.upload.upload);
        uploadBtnModal.setAttribute('id', 'arcspy-upload-button');
        uploadBtnModal.addEventListener('click', () => {
            ...
            if (cachedScores == null || cachedScores[player.user_id] === undefined) {
                alert(i18n.player.scoreNotFound);
                return;
            }
            ...
            fetch("https://arcspy.lxns.net/player/sync", {
                ...
                response => {
                    ...
                    if (response.code != 200) {
                        alert(replaceVariables(i18n.upload.uploadFailed, { errorMessage: response.message }))
                    } else {
                        alert(i18n.upload.uploadSuccess);
                    }
                }
            ).catch(error => alert(i18n.upload.serverUnavailable));
        });
        modalAction.appendChild(uploadBtnModal);
        ...
        arcSpyModal.setModalBody(i18n.upload.title, modalBody);
    }
    ...
    */
    function openExportModal() {
        ...
        bodyModal.appendChild(arcSpyModal.createSpanModal(i18n.export.selectTargetPlayer));
        ...
        bodyModal.appendChild(arcSpyModal.createSpanModal(i18n.export.exportNotice));

        const modalAction = arcSpyModal.getInitializedModalAction();

        const exportCsvBtnModal = arcSpyModal.createBtnModal(i18n.export.exportAsCsv);
        ...
        arcSpyModal.setModalBody(i18n.export.title, modalBody);
    }
    ...
    function openDisclaimerModal() {
        ...
        bodyModal.appendChild(arcSpyModal.createSpanModal(i18n.disclaimer.line_1));
        bodyModal.appendChild(arcSpyModal.createSpanModal(i18n.disclaimer.line_2));
        bodyModal.appendChild(arcSpyModal.createSpanModal(i18n.disclaimer.line_3));
        bodyModal.appendChild(arcSpyModal.createSpanModal(i18n.disclaimer.line_4));

        const modalAction = arcSpyModal.getInitializedModalAction();
        
        const agreeBtnModal = arcSpyModal.createBtnModal(i18n.disclaimer.agree, 'light');
        agreeBtnModal.addEventListener('click', () => {
            localStorage.setItem("disclaimer", "true");
            openMainModal();
        });
        modalAction.appendChild(agreeBtnModal);

        const disagreeBtnModal = arcSpyModal.createBtnModal(i18n.disclaimer.disagree);
        disagreeBtnModal.addEventListener('click', arcSpyModal.closeModal);
        modalAction.appendChild(disagreeBtnModal);

        modalBody.appendChild(bodyModal);
        modalBody.appendChild(modalAction);

        arcSpyModal.createModal(i18n.disclaimer.title, modalBody);
    }
    ...
    */
    function openAboutModal() {
        ...
        const exportBtnModal = arcSpyModal.createBtnModal(i18n.export.title, 'light');
        exportBtnModal.addEventListener('click', openExportModal);
        modalAction.appendChild(exportBtnModal);

        const resetBtnModal = arcSpyModal.createBtnModal(i18n.about.reset, 'light');
        resetBtnModal.style.color = "red";
        resetBtnModal.addEventListener('click', () => {
            localStorage.clear();
            alert(i18n.about.resetSuccess);
        ...
        arcSpyModal.setModalBody(i18n.about.title, modalBody);
    }
    ...
    */
    function openMainModal() {
        ...
        bodyModal.appendChild(arcSpyModal.createSpanModal(i18n.selectFunction, 'center'));

        const modalAction = arcSpyModal.getInitializedModalAction();

        const saveBtnModal = arcSpyModal.createBtnModal(i18n.cachePlayerDetails);
        saveBtnModal.addEventListener('click', () => {
            saveBtnModal.classList.add('disabled');
            requestAPI('GET', 'user/me').then((response) => {
                if (!response.success) {
                    alert(replaceVariables(i18n.cacheFailed, { errorCode: response.error_code }));
                    return;
                }
                savePlayerInfo(response.value);
                alert(replaceVariables(i18n.cacheSuccess, { playerName: response.value.name }))
        ...
        const scoreQueryBtnModal = arcSpyModal.createBtnModal(i18n.query.title);
        scoreQueryBtnModal.addEventListener('click', openScoreQueryModal);
        modalAction.appendChild(scoreQueryBtnModal);

        const uploadBtnModal = arcSpyModal.createBtnModal(i18n.upload.title);
        uploadBtnModal.addEventListener('click', openUploadModal);
        modalAction.appendChild(uploadBtnModal);

        const optionBtnModal = arcSpyModal.createBtnModal(i18n.config.title);
        optionBtnModal.addEventListener('click', openOptionModal);
        modalAction.appendChild(optionBtnModal);

        const aboutBtnModal = arcSpyModal.createBtnModal(i18n.about.title);
        ...
        copyrightSpan.style.position = "absolute";
        copyrightSpan.style.right = "0";
        modalBody.appendChild(copyrightSpan);

        if (arcSpyModal.modalBody !== null) {
            arcSpyModal.setModalBody(i18n.title, modalBody);
            return;
        }
        arcSpyModal.createModal(i18n.title, modalBody);
    }
